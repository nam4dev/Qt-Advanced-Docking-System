...
namespace ads {
    ...
    struct DockWidgetPrivate {
         ...
         */
        bool createWidgetFromFactory() const;

        /**
         * Use the dock manager toolbar style and icon size for the different states
         */
        void setToolBarStyleFromDockManager() const;
    };
    ...
    void DockWidgetPrivate::showDockWidget() {
        ...
        if (!DockArea) {
            ...
            if (Container->openedDockWidgets().count() == 0 && DockArea->isAutoHide() && !DockManager->
                isRestoringState()) {
    ...
    //============================================================================
    bool DockWidgetPrivate::createWidgetFromFactory() const {
        if (!Features.testFlag(CDockWidget::DeleteContentOnClose)) {
            return false;
        }
    ...
    //============================================================================
    void DockWidgetPrivate::setToolBarStyleFromDockManager() const {
        if (!DockManager) {
            return;
        }
    ...
    //============================================================================
    CDockWidget::CDockWidget(const QString &title, QWidget *parent) : QFrame(parent),
    ...
    void CDockWidget::contextMenuHandler(QContextMenuEvent *ev) {
    }
    ...
    void CDockWidget::setDockManager(CDockManager *DockManager) {
        ...
        d->TabWidget = DockManager->getFactory()->createDockWidgetTab(this);
    ...
    //============================================================================
    void CDockWidget::setAsCurrentComponent() {
        // This case occurs only when the current tab
        // is shifted due to auto hide docking being pinned.
        if (isCurrentTab()) {
            toggleView(false);
            toggleView(true);
        }
    ...
    void CDockWidget::setAutoHide(bool Enable, SideBarLocation Location, int TabIndex) {
        ...
        if (!Enable) {
            ...
            Q_EMIT pinned(true, Location, -1);
        } else if (isAutoHide()) {
            autoHideDockContainer()->moveToNewSideBarLocation(Location);
            Q_EMIT pinned(false, Location, -1);
        } else {
            ...
            Q_EMIT pinned(false, area, TabIndex);
        }
    }
